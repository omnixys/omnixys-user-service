...
> Repository development instructions for **user** (Service).
> Stack: nestjs. Generated as part of the Omnixys repository-policy rollout.

## Repository Overview

Omnixys User Service – user profiles, admin, analytics.
...
src/adapter, admin, analytics, config, core, handlers, prisma, security, user
...
node --test __tests__/unit/*.test.mjs and __tests__/integration/*.test.mjs; Jest e2e for resolvers
