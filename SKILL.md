<!-- repository: services/user | kind: SERVICE | stack: nestjs -->

# user — Skill: Service Development

> Workflow for user (services/user). Execute this workflow before, during, and
> after changes in this repository.

## Repository Facts

- Kind: Service
- Package: `user-service` (version: 3.3.0)
- Runtime: Node >=25.8.2 (pnpm >=10.33.0)
- Description: Omnixys User Service – user profiles, admin, analytics.
- Architecture: src/adapter, admin, analytics, config, core, handlers, prisma, security, user
...
- Tests: node --test __tests__/unit/*.test.mjs and __tests__/integration/*.test.mjs; Jest e2e for resolvers
...
- Identify the affected bounded context within `src/adapter, admin, analytics, config, core, handlers, prisma, security, user`.
...
- Keep tenant isolation intact (`Personal data: privacy, PII handling, and tenant isolation are critical.`).
